...
This is the **SAM Website** - the official documentation site for SAM, CLIO, and ALICE.

**Verification sources for technical facts:**
- SAM features: `../SAM/Sources/` (not always available locally; treat SAM source as authoritative)
- CLIO features: `../CLIO/lib/CLIO/` (the project sibling)
- ALICE features: `../ALICE/` (the project sibling)

When documenting a feature, prefer to verify against actual source code rather than memory. If a fact cannot be verified, mark the doc with `[NEEDS VERIFICATION]` and ask the user.

**The site is HTML-first:** Documentation pages are committed as `.html` files under `docs/SAM/`, `docs/CLIO/`, `docs/ALICE/`, and `docs/shared/`. There is no markdown→HTML build step in the deployment pipeline. Edit the HTML directly.
